...
render.options.showAngleIndicator = true; //角度制式
render.options.showVelocity = true; //速度
...
function rod(ball, rod) {
    ...
    var positionOriX,positionOriY;//position球杆位置click点击位置
    var stepX,stepY;
    var angleRod = 0;//球杆角度
    ...
    //击打动作
    function impact() {
        console.log(stepX,positionX < positionOriX)
        positionX = positionX + stepX;
        positionY = positionY + stepY;
        if (stepX > 0 && positionX > positionOriX) {
            Body.setPosition(rod, {
                x: positionOriX,
                y: positionOriY
            });
        ...
        else if (stepX < 0 && positionX < positionOriX) {
            Body.setPosition(rod, {
                x: positionOriX,
                y: positionOriY
            });
    ...
    Events.on(mouseConstraint, "mousedown", function (event) {
        ...
        positionOriX = rod.position.x;
        positionOriY = rod.position.y;
    ...
    Events.on(mouseConstraint, "mouseup", function (event) {
        ...
        {
            if(positionX > positionOriX){
                stepX = -1;
            }
            else{
                stepX = 1;
            }
            if(positionY > positionOriY){
                stepY = -Math.abs(stepX/Math.cos(angleRod)*Math.sin(angleRod));
            }
            else{
                stepY = Math.abs(stepX/Math.cos(angleRod)*Math.sin(angleRod));
            }
